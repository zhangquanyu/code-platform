# 4p12s-delivery-orchestrator 技能
...
本技能是端到端交付系统的**初始化器**。它**只做一件事**：在指定交付目录下创建并初始化 `delivery-state.md` 状态账本。
...
- current_task:
- blocking_reason:
- next_action: 等待用户或编排器触发 4p12s-requirements 技能
- max_repair_rounds: 8
...
**4p12s-delivery-orchestrator** 是端到端交付系统的初始化器，负责创建并初始化 `delivery-state.md` 状态账本，区分流程类型，支持增量交付和修复轮次弹性配置。
