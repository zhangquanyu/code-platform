# 4p12s-lightweight-flow 技能（轻量化流程）
...
- **技能定位**: 简单变更的精简交付流程
- **版本**: v2.0.0（新增）
- **文件路径**: `docs/4p12s-skills-v2/00-lightweight-flow.md`

---

## 一、概述

本 Skill 是 4P12S v2 **新增**的轻量化交付流程，适用于不需要完整 12 步流程的简单变更场景。它将完整流程精简为 **4 个步骤**，在保证基本质量的前提下大幅降低流程成本。
...
如果在轻量化流程执行过程中发现变更比预期复杂，可以升级到完整流程：
...
在执行轻量化流程时，如果发现以下情况，应建议用户升级到完整流程：
...
- ❌ 不得用轻量化流程处理新功能开发
- ❌ 不得跳过基础验证直接推送
- ❌ 不得在轻量化流程中执行 E2E 测试（应升级到完整流程）
- ❌ 不得在轻量化流程中触发发布（轻量化流程只到 Git 推送为止）
...
**4p12s-lightweight-flow** 是 4P12S v2 的轻量化交付流程，适用于简单变更场景。它将 12 步流程精简为 4 步，在保证基本质量的前提下大幅降低流程成本。
