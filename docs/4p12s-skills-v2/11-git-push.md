...
- **技能定位**: Git 推送
- **版本**: v2.0.0
- **文件路径**: `docs/4p12s-skills-v2/11-git-push.md`
...
- ✅ **commit-msg 规范统一英文冒号**，消除中文全角冒号混淆风险
- ✅ 简化任务信息提取流程
- ✅ deploy-log.md 记录格式优化
...
### 4.1 v2 commit-msg 格式

```
${commit}:${taskId}_${name}:${submitDescription}
```

> ⚠️ **v2 关键改进**: 所有冒号统一为**英文半角冒号 `:`**，消除 v1 中文全角冒号 `：` 导致的混淆和 husky hook 校验失败问题。
...
| 版本 | 格式 | 冒号类型 | 风险 |
|------|------|----------|------|
| v1 | `${commit}:${taskId}_${name}：${submitDescription}` | 混合（英文 + 中文全角） | 全角空格、编码问题 |
| v2 | `${commit}:${taskId}_${name}:${submitDescription}` | 统一英文半角 | 无混淆风险 |
...
3. 仅暂存这些明确路径
4. **【必须】提取任务信息**
5. 按 commit-msg 格式模板直接填充组装（v2 统一英文冒号）
...
- remote:
- staged_files:
- commit_message:         # 完整 commit-msg（v2 英文冒号）
- commit_command:
- commit_exit_code:
- push_command:
- push_exit_code:
